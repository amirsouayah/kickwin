...
function LeagueEast() {
  ...
  const removeTeam = (index) => {
    swal({
      title: "Are you sure?",
      text: "Are you sure that you want to remove this Team",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    })
      .then(willDelete => {
        if (willDelete) {
          axios.put(`http://localhost:5000/league/deleteleague/${id}`, { teams: [index] })
            .then(res => {
              if (res.status === 200) {
                  ...
                  >
                    ...
                    <Grid item xs="auto">
                      <Box
                        ...
                        flexWrap="wrap"
                      >
                        {data && <AddNewMatch dataTeam={data[0].teams} sendData='create' />}
                      </Box>
                    </Grid>
                  </Grid>
                }
